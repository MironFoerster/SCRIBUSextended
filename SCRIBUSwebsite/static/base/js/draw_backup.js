...
class pathElement {
    ...
    // der Koordinatenursprung zu dem der Pfad relativ ist,
    // die
...
const Tstart = (evt) => {
    ...
    if (global.freehand == true) {
        ...
        if (global.focusedEl != undefined) { //one element is focused
            ...
            if (pointIsOnArea(global.posOnCvs, rsButtonCenter, 30, 30)) { //touchstart is on the rotscale button of the focused element
            ...
            for (let el of global.elements) {
                ...
                for (let partpath of el.path) {
                    ...
                    for (let point of partpath) {
                        // for all horizontal or vertical strokes
                        if (prevPoint[0] == point[0]) { // vertical
                        ...
                        } else if (prevPoint[1] == point[1]) { // horizontal
                            ...
                            // for all non-horizontal or non-vertical strokes
                            let m = (prevPoint[1]-point[1])/(prevPoint[0]-point[0])  // deltaY/deltaX
                            let n = point[1] - m * point[0]  // y-m*x
                            let mOrth = -1/m
                            let nOrth = touchOnEl.y - mOrth * touchOnEl.x  // y-m*x
                            // standard lösung eines linearen Gleichungssystems
                            let intersectX = (n-nOrth)/(mOrth-m)
                            let intersect = {'x': intersectX, 'y': m * intersectX + n}

                            // add the distance only if the intersection is on the stroke
...
// Wenn die Seite geladen ist: canvas zeichnen,
